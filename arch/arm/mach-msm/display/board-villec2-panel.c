...
static int mipi_pyramid_device_register(const char* dev_name, struct msm_panel_info *pinfo, u32 channel, u32 panel);
#if defined (CONFIG_MSM_AUTOBL_ENABLE)
static int pyramid_samsung_acl_enable(int on, struct msm_fb_data_type *mfd);
#endif
...
#if defined (CONFIG_MSM_AUTOBL_ENABLE)
	.autobl_enable = pyramid_samsung_acl_enable,
#endif
};
...
static int mipi_cmd_samsung_blue_qhd_pt_init(void)
{
	...
	ret = mipi_pyramid_device_register("mipi_samsung", &pinfo, MIPI_DSI_PRIM,
...
static inline void pyramid_mipi_dsi_set_backlight(struct msm_fb_data_type *mfd)
{
...
static int pyramid_lcd_on(struct platform_device *pdev)
{
	...
	if (mipi->mode == DSI_VIDEO_MODE) {
		...
		if (!mipi_lcd_on) {
			...
			if (panel_type == PANEL_ID_VILLE_SAMSUNG_SG_C2) {
				printk(KERN_INFO "pyramid_lcd_on PANEL_ID_VILLE_SAMSUNG_SG_C2\n");
...
static void pyramid_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	pyramid_mipi_dsi_set_backlight(mfd);
...
static int __init pyramid_panel_late_init(void)
{
...
module_init(pyramid_panel_init);
late_initcall(pyramid_panel_late_init);
